"""
Trial 4 training — Qwen3-VL-32B nf4 QLoRA, masked SFT with presentation-
shuffle augmentation. Built for a single 80GB+ GPU (vast.ai), inside tmux.

Trial4 change vs trial3: the checkpoint monitor can use the REAL decision
rule (24-way constrained scoring, tta_k=1) instead of greedy decoding.
Trial3's 80-row greedy monitor had ~+-5.3%p noise — checkpoint selection was
effectively random. SNU_MONITOR_MODE=score (default) fixes the metric;
SNU_MONITOR_N sizes the noise (150 rows ~ +-3.7%p, 300 ~ +-2.6%p).

Epoch-2 resume path (trial3 ckpt-400): the original run's cosine schedule
was already built for SNU_EPOCHS=2 (1016 steps; lr at step 506 was 5.3e-5 =
half peak), so resuming with the same SNU_EPOCHS/SNU_LR/SNU_SEED/TRAIN_CSV
continues the planned schedule exactly. Dataloader fast-forward skips
in_epoch_step*ACCUM*MB rows — row skip count is invariant to the MB/ACCUM
split as long as MB*ACCUM stays 16.

Env knobs:
  SNU_TRAIN_CSV=data_work/train_clean.csv   SNU_VAL_CSV=data_work/clean_val.csv
  SNU_CKPT_DIR=ckpts   SNU_MB=4   SNU_ACCUM=4   SNU_EPOCHS=2   SNU_LR=1e-4
  SNU_LORA_R=32   SNU_SAVE_STEPS=150   SNU_MONITOR_N=150   SNU_SEED=42
  SNU_MONITOR_MODE=score|greedy
"""
import hashlib
import json
import os
...
from common import (SIGMAS, TRAIN_IMG_DIR, build_messages,
                    ...
                    lora_target_modules, predict_order, slot_order,
                    target_text)
...
MB = int(os.environ.get("SNU_MB", "4"))
...
@torch.no_grad()
def score_monitor(model, processor, val_rows, n):
    """Exact-match under the REAL decision rule (24-way scoring, tta_k=1,
    identity presentation, no prior). ~3-4x slower per row than greedy but
    measures the deployed metric instead of a proxy."""
    model.eval()
    correct = 0
    rows = val_rows[:n]
    for row in rows:
        pred = predict_order(model, processor, row, TRAIN_IMG_DIR, tta_k=1)
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        while True:
            ...
            if step % SAVE_STEPS == 0:
                ...
                monitor = (score_monitor if MONITOR_MODE == "score"
                           else greedy_monitor)
